...
const getDetailedCartList = (uid: any) => {
...
Mock.onGet('/api/ecommerce/get-product-list').reply((config: any) => {
  const response = EcommerceDB.productList;
  return [200, response];
});

Mock.onGet('/api/ecommerce/get-category-list').reply((config: any) => {
  const response = EcommerceDB.category;
  return [200, response];
});

Mock.onGet('/api/ecommerce/get-rating-list').reply((config: any) => {
  const response = EcommerceDB.rating;
  return [200, response];
});

Mock.onGet('/api/ecommerce/get-brand-list').reply((config: any) => {
  const response = EcommerceDB.brand;
  return [200, response];
});

Mock.onGet('/api/ecommerce/get-cart-list').reply((config: any) => {
  let uid = config.data;
  let response: any = [];
...
Mock.onPost('/api/ecommerce/add-to-cart').reply((config: any) => {
...
Mock.onPost('/api/ecommerce/delete-from-cart').reply((config: any) => {
  let { uid, productId } = JSON.parse(config.data);
...
Mock.onPost('/api/ecommerce/update-cart-amount').reply((config: any) => {
