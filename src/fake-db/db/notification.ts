...
Mock.onGet('/api/notification').reply((config: any) => {
  const response = NotificationDB.list;
  return [200, response];
});

Mock.onPost('/api/notification/add').reply((config: any) => {
  const response = NotificationDB.list;
  return [200, response];
});

Mock.onPost('/api/notification/delete').reply((config: any) => {
...
Mock.onPost('/api/notification/delete-all').reply((config: any) => {
  NotificationDB.list = [];
  const response = NotificationDB.list;
  return [200, response];
});
